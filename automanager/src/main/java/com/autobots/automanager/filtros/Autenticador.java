...
public class Autenticador extends UsernamePasswordAuthenticationFilter {
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
		...
		try {
	        ...
	        // Adiciona a lógica para determinar o tipo de credencial com base no campo "tipo"
	        if ("UsuarioSenha".equalsIgnoreCase(credencial.getTipo())) {
	            credencial = new ObjectMapper().readValue(request.getInputStream(), CredencialUsuarioSenha.class);
	        } else if ("CodigoBarra".equalsIgnoreCase(credencial.getTipo())) {
	            credencial = new ObjectMapper().readValue(request.getInputStream(), CredencialCodigoBarra.class);
	        } else {
	            // Lógica para lidar com o caso em que o tipo de credencial não é reconhecido
	            throw new IllegalArgumentException("Tipo de credencial não reconhecido");
	        }
	    } catch (IOException e) {
